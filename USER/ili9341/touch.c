...
void TP_Save_Adjdata(void)
{
    ...
    buf[8]  = (tp_dev.xoff >> 8) & 0xFF;
    buf[9]  = tp_dev.xoff & 0xFF;
    buf[10] = (tp_dev.yoff >> 8) & 0xFF;
    buf[11] = tp_dev.yoff & 0xFF;
    buf[12] = tp_dev.touchtype;
    buf[13] = 0x0A;   // 标志位，表示数据有效

    // 写入 FLASH
    W25QXX_Write(buf, SAVE_ADDR_BASE, 14);
...
uint8_t TP_Get_Adjdata(void)
{
    ...
    // 读出存储的数据
    W25QXX_Read(buf, SAVE_ADDR_BASE, 14);

    if(buf[13] == 0x0A)  // 校验标志位
    {
        tempfac = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
        tp_dev.xfac = (float)tempfac / 100000000;

        tempfac = (buf[4] << 24) | (buf[5] << 16) | (buf[6] << 8) | buf[7];
        tp_dev.yfac = (float)tempfac / 100000000;

        tp_dev.xoff = (buf[8] << 8) | buf[9];
        tp_dev.yoff = (buf[10] << 8) | buf[11];
